...
def admin_users_create(
    ...
    if not username:
        return RedirectResponse(url='/admin/users?error=Zadej%20uživatelské%20jméno.', status_code=303)
    if '@' not in email:
        return RedirectResponse(url='/admin/users?error=Zadej%20platný%20e-mail.', status_code=303)
    if len(password) < 8:
        return RedirectResponse(url='/admin/users?error=Heslo%20musí%20mít%20alespoň%208%20znaků.', status_code=303)

    with SessionLocal() as db:
        if get_user_by_email(db, email):
            return RedirectResponse(url='/admin/users?error=Tento%20e-mail%20už%20existuje.', status_code=303)
        if get_user_by_username(db, username):
            return RedirectResponse(url='/admin/users?error=Toto%20uživatelské%20jméno%20už%20existuje.', status_code=303)
        create_user(db, username=username, email=email, password=password, role=role)
    return RedirectResponse(url='/admin/users?success=Uživatel%20byl%20vytvořen.', status_code=303)


@router.post('/users/{user_id}/toggle-active')
def admin_users_toggle_active(request: Request, user_id: str):
    if not require_admin(request):
        return RedirectResponse(url='/login', status_code=303)
    with SessionLocal() as db:
        user = db.get(User, user_id)
        if not user:
            return RedirectResponse(url='/admin/users?error=Uživatel%20neexistuje.', status_code=303)
        if getattr(request.state.current_user, 'id', None) == user.id and user.role == 'admin' and user.is_active:
            return RedirectResponse(url='/admin/users?error=Nemůžeš%20deaktivovat%20svůj%20vlastní%20admin%20účet.', status_code=303)
        user.is_active = not user.is_active
        db.add(user)
        db.commit()
    return RedirectResponse(url='/admin/users?success=Stav%20uživatele%20byl%20změněn.', status_code=303)
